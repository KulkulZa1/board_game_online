...
### Fixed — four balance-corrupting bugs found by a measured audit of all nine arcade games
- **Snake and breakout shared the same bug, with the same 3.8× effect.** `draftOffers()` filtered the pool by `run.owned` alone, but `grant()` *removes* materials from `owned` when they evolve/fuse — so consumed materials returned to the draft pool and the same evolution could be farmed forever. Measured: snake score median 1,234,071 → 4,693,745 and evolutions median 28 per run (there are only 5 distinct evolutions); breakout level median 76.5 → 291. Both now track a `consumed` list, and both suites assert that 500 draws never re-offer a spent material.
- **NEON CASCADE rounds could not end.** The time-orb bonus added the amp's `timeBonus` *outside* the decay — defeating the decay the comment directly above it declares as the thing that ends late rounds. Measured: a full amp stack left **25/25 rounds unfinished at 3,000 seconds** (wave 1702 observed). Moving the bonus inside the decay was not enough (it just pushed the dry-out past wave 50) and multiplying was not enough either (it scaled the floor too). Amps now scale only the decaying part while the floor stays an absolute constant: full-stack rounds finish at a 141s median, **0/25 unfinished**.
- **첨탑 대란's life-cost curse was free.** `Math.max(1, lives + livesCap)` meant 과부하 코어 (+35% damage, −3 lives) cost nothing at low lives, and 1.35ⁿ then outruns the 1.34ⁿ enemy HP curve — measured **80/80 forced-pick runs surviving past wave 60**. The floor stays (it prevents suicide-on-pick), but the draft no longer offers a life-cost curse the player cannot actually pay.
- All four were found by a 15-agent audit that swept every arcade game's headless sim (jackpot n=1,000; tower-defense 380+; breakout 720; neon-cascade 2,438 rounds; bootstrap 200 seeds; factory 20,000). Suites grew: snake 45→49, breakout 41→45, neon 37→41, tower-defense 76→80.
- Lobby: snake and breakout cards still described the pre-roguelite games; updated to mention drafting and fusion.
...
- **The build phase has a countdown that auto-launches the wave** (`BUILD_SECONDS`: 12.7s at wave 1 → 6s floor), with the last 3 seconds beeping and the wave button pulsing. Calling early converts the remaining seconds into gold. The bonus was originally ~2× a wave's income, which made calling early *mandatory* — a patient bot went bankrupt at wave 4 while a greedy one reached 23; resized so patience is worth ~2 extra waves (19.6 vs 12.1 with human-like hands) and speed buys tempo instead.
- **Kill combo with tiers** (15/45/100 → ×1.2/1.4/1.7) that a single leak wipes, so every leaked enemy costs twice. First tuning used 8/20/40 with a 2.6s window and never broke — a measured peak streak of **305**, i.e. a permanent ×3 economy rather than a tension device.
- **Bosses accelerate as they lose HP** — a boss you have nearly killed can sprint the last stretch.
- **Fused towers now level with no cap.** With a cap, the board saturated and late gold became worthless (measured: **31,761 unspent gold on a completely full board**), which silently killed every economy decision and made every run end at the same wave. Cost grows ×2.2 per level against ×1.62 damage, so gold stays scarce and the wall stays real.
- **The meta shop was dead and is now multiplicative.** Flat bonuses (start gold, lives) moved the wall by **0.5 waves total** against exponential enemy HP. 단조 화력 (damage ×) and 전리품 감식 (bounty ×) replace them; full meta is now worth **+2.6 waves**, asserted in the suite.
- **UI overhaul**: persistent tower palette (pick once, place repeatedly — the old flow needed two taps per placement), a countdown ring on the launch button showing time and the early-call bonus, a red threat vignette and pulsing gate as enemies near the exit, a heartbeat animation on low lives, boss HP bar, combo gauge, per-tower popover with live DPS, keyboard shortcuts (1-6 / space / P / S / M), pause, and a run-summary screen. Three real bugs were caught by screenshotting: the path was nearly invisible, tower icons were drawn in the last grid colour (i.e. not drawn at all), and the palette toggled off when tapping the already-selected tower.
- `td-rogue-test.js` grew 45 → 76 assertions covering pacing, countdown, early-call sizing, combo tiers/decay, boss rage, threat, and uncapped fusion.
- Verified in real browser play driving the DOM: aggressive play reaches wave 21 in **3.3 minutes** (9.4s/wave), patient play wave 18 in 5.2 minutes, with leaks happening throughout — no longer a run where nothing threatens you.
