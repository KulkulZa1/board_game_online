...
The diagnostics branch is `audit/version-diagnostics`. It is stacked on earlier launch-readiness/cache work that is not present in the local `origin/main` snapshot used during this pass. If the PR view includes more than this diagnostics commit, merge or retarget the earlier launch-readiness branches first, or keep this as a stacked PR intentionally.
