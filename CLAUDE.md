...
| `npm run test:games` | `scripts/run-game-flow-tests.js` — runs 8 rule-engine/flow suites from `prototypes/`: Mahjong (engine, flow, timer), BANG! flow, a 36-assertion backgammon/texasholdem/dotsboxes handler suite, a 40-assertion suite covering the other 9 board games (`core-games-handler-test.js`), and the two arcade roguelite suites `snake-rogue-test.js` (42) and `breakout-rogue-test.js` (39), which assert those games' rules *and* balance. Each is also runnable standalone via `node prototypes/<name>.js` |
