...
| `npm run test:games` | `scripts/run-game-flow-tests.js` — runs 12 rule-engine/flow suites from `prototypes/`: Mahjong (engine, flow, timer), BANG! flow, a 36-assertion backgammon/texasholdem/dotsboxes handler suite, a 40-assertion suite covering the other 9 board games (`core-games-handler-test.js`), and the six arcade progression suites `snake-rogue-test.js` (42), `breakout-rogue-test.js` (39), `neon-amp-test.js` (32), `plant-prestige-test.js` (52), `jackpot-meta-test.js` (68) and `jackpot-map-test.js` (33), which assert those games' rules *and* balance. Each is also runnable standalone via `node prototypes/<name>.js` |
