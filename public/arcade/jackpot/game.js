...
(function () {
  ...
  const { Run, SYMBOLS, COLS, CELLS, SPINS_PER_RENT, WIN_STAGE, SKIP_COINS, rentFor } = window.Jackpot;
  ...
  function setCell(i, item) {
    ...
    sym.textContent = item ? SYMBOLS[item.id].icon : '';
  ...
  function renderHUD() {
    ...
    for (let i = 0; i < SPINS_PER_RENT; i++) {
      const d = document.createElement('span');
      d.className = 'sdot' + (i < run.spinsIntoStage ? ' done' : '');
      dots.appendChild(d);
    }
    // 마지막 스핀인데 코인이 부족하면 위험 펄스
    const danger = run.spinsIntoStage === SPINS_PER_RENT - 1 && run.coins < run.rent();
  ...
  function finishSpin(result) {
    const pl = $('payLine');
    pl.textContent = result.total > 0 ? `이번 스핀 +${result.total}` : '꽝... 다음 스핀!';
    if (result.total >= 40) { pl.classList.add('mega'); pl.textContent = `🔥 대박! +${result.total}`; }
    renderHUD();

    if (result.settle) {
      setTimeout(() => showSettle(result.settle), T(450));
    } else {
      setTimeout(() => showPick(false), T(500));
    }
  }
  ...
  function showSettle(settle) {
    ...
    if (settle.type === 'won') {
      ...
      setTimeout(() => showPick(false, settle.bonus), T(1000));
    }
  }
  ...
  function showPick(isBonus, queueBonus) {
    ...
    $('skipBtn').innerHTML = `건너뛰고 +${SKIP_COINS} 코인 <span class="key-hint">0</span>`;
  ...
  function init() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      } else if (phase === 'pick' && ['1', '2', '3'].includes(e.key)) {
        const btns = document.querySelectorAll('#pickCards .pick-card');
        const b = btns[parseInt(e.key, 10) - 1];
        if (b) { e.preventDefault(); b.click(); }
      } else if (phase === 'pick' && e.key === '0') {
