...
| **tower-defense** | `/arcade/tower-defense/` | 첨탑 대란 — path-defense **roguelite**, rebuilt from scratch (the old page was the sandbox editor wearing a game's URL; a bot lost with 0 waves cleared and the verdict was "not fun"). 6 towers × 3 levels, adjacent same-type Lv3 pairs **fuse** (consuming one) into 6 evolved towers; every cleared wave drafts 1 of 3 loot cards (perks + curses); bosses every 5th wave; wave preview enables planning. 🔮 mana cores (√score payout) buy 5 permanent studies. Headless `sim.js`, `td-rogue-test.js` pins rules *and* the wave-wall (bot median ~24, no infinite survival) | `COLS=7, ROWS=10`; `TOWERS(6), FUSIONS(6), PERKS(9), CURSES(3), META_UPGRADES(5)` in `sim.js` | `td_meta_v1`, `td_muted` |
